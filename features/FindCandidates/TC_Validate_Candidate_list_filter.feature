...
Feature: Validate Candidate list filter
  ...
  Scenario Outline: Validate Candidates filter modal layout
    ...
    And user sees "FILTER CANDIDATE" as the title of candidate list filter modal
  ...
  Scenario: Validate Candidates filter Apply button
    ...
    And user selects "CAD Leader" as an option for "Job role" on candidate list filter modal
    And user selects "UK - SE & London" as an option for "Location" on candidate list filter modal
